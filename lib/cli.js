...
(function (factory) {
    if (typeof module === "object" && typeof module.exports === "object") {
        ...
        define(["require", "exports", "chalk", "path", "glob", "yargs-parser", "lodash.uniq", "assemblyscript/cli/asc", "./test/TestContext", "fs", "assemblyscript/lib/loader", "./reporter/DefaultTestReporter"], factory);
    }
    ...
    function asp(args) {
        ...
        if (yargs.argv.i || yargs.argv.init) {
            ...
            var testEntryFiles_1 = [];
            var addedTestEntryFiles_1 = [];
            // for each pattern
            for (var _i = 0, include_1 = include; _i < include_1.length; _i++) {
                var pattern = include_1[_i];
                // push all the resulting files so that each file gets tested individually
                testEntryFiles_1.push.apply(testEntryFiles_1, glob_1.default.sync(pattern));
            }
            for (var _a = 0, add_1 = add; _a < add_1.length; _a++) {
                var pattern = add_1[_a];
                // push all the added files to the added entry point list
                addedTestEntryFiles_1.push.apply(addedTestEntryFiles_1, glob_1.default.sync(pattern));
            }
            // remove duplicate file locations
            testEntryFiles_1 = lodash_uniq_1.default(testEntryFiles_1);
            // run the regex tests to find excluded tests
            disclude.forEach(function (regexp) {
                testEntryFiles_1 = testEntryFiles_1.filter(function (file) { return !regexp.test(file); });
            });
            ;
            ...
            addedTestEntryFiles_1.push(relativeEntryPath);
            // Create a test runner, and run each test
            var failed_1 = false;
            var count_1 = testEntryFiles_1.length;
            var runner_1 = new TestContext_1.TestContext();
            ...
            testEntryFiles_1.forEach(function (file, i) {
                console.log("Compiling: " + file + " " + (i + 1).toString() + " / " + testEntryFiles_1.length.toString());
                asc_1.default.main([file].concat(addedTestEntryFiles_1, flagList_1), {
                    ...
                    writeFile: function (name, contents) {
                        ...
                        if (path_1.default.extname(name) === ".map") {
                    ...
                    var imports = runner_1.createImports(configuration_1.imports || {});
                    var wasm = loader_1.instantiateBuffer(binaries_1[i], imports);
                    // call run buffer because it's already compiled
                    runner_1.run(wasm, reporter_1);
                    count_1 -= 1;
                    failed_1 = failed_1 || !runner_1.pass;
