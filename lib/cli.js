...
files.forEach(function (file, i) {
    asc_1.default.main([
        ...
        if (error) {
            console.log(chalk_1.default(templateObject_6 || (templateObject_6 = __makeTemplateObject(["{bgRedBright.black [Error]} There was a compilation error when trying to create the wasm binary for file: ", "."], ["{bgRedBright.black [Error]} There was a compilation error when trying to create the wasm binary for file: ", "."])), file));
            console.log(error);
            process.exit(1);
            return;
        }
        if (!binaries[i]) {
            console.log(chalk_1.default(templateObject_7 || (templateObject_7 = __makeTemplateObject(["{bgRedBright.black [Error]} There was no output binary file: ", "."], ["{bgRedBright.black [Error]} There was no output binary file: ", "."])), file));
            process.exit(1);
            return;
        }
