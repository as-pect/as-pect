(function (factory) {
    ...
    function getString(wasm, pointer) {
        var start = pointer >>> 1;
        var length = wasm.U32[(pointer >>> 2) - 3] >> 1;
        // @ts-ignore
    ...
    var TestContext = /** @class */ (function () {
        ...
        TestContext.prototype.runGroup = function (runContext, group) {
            ...
            var groupName = group.describePointers
                .map(function (pointer) { return getString(runContext.wasm, pointer); })
                .join(" ");
            group.name = groupName;
            runContext.endGroup = false;
            for (var _i = 0, _a = group.todoPointers; _i < _a.length; _i++) {
                var todoPointer = _a[_i];
                var todo = getString(runContext.wasm, todoPointer);
