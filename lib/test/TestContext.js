    ...
    var TestContext = /** @class */ (function () {
        ...
        TestContext.prototype.runTest = function (runContext, group, testIndex) {
            ...
            if (performanceEnabled) {
                ...
                var minTestRuntime = !isFinite(minTestRuntimeValue)
                ...
                while (true) { // always run at least once
                    ...
                    if (runContext.endGroup)
                        return;
                    var runTime = perf_hooks_1.performance.now() - start;
                    if (maxSamples !== void 0 && runCount >= maxSamples)
                        break; // if we have reached the max sample count
                    if (maxTestRuntime !== void 0 && runTime >= maxTestRuntime)
                        break; // weve collected enough samples and the test is over
                    if (minSamples !== void 0 && runCount < minSamples)
                        continue; // running if we haven't reached the minimum sample count
                    if (minTestRuntime !== void 0 && runTime < minTestRuntime)
                        continue; // running if we can run more samples for this test
                }
                if (reportAverage)
         ...
         */
        TestContext.prototype.minSamples = function (value) {
         ...
         */
        TestContext.prototype.minTestRunTime = function (value) {
