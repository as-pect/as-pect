...
export function init(assemblyFolder: string, testFolder: string, typesFile: string, typesFileSource: string) {
  ...
  if (!fs.existsSync(testFolder)) {
    ...
    if (!fs.existsSync(exampleFile)) {
      console.log(chalk`{bgWhite.black [Log]} Creating file: {yellow ./assembly/__tests__/example.spec.ts}`);
  ...
  if (!fs.existsSync(configFile)) {
    console.log(chalk`{bgWhite.black [Log]} Creating file: {yellow as-pect.config.js}`);
