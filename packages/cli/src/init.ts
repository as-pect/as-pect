...
export async function init() {
  ...
  if (!existsSync(assemblyFolder)) {
    console.log(`${chalk.bgWhite.black("[Log]")} Creating folder: ${chalk.yellow("./assembly/")}`);
    await fs.mkdir(assemblyFolder);
  }
  // Create the test folder if it doesn't exist
  if (!existsSync(testFolder)) {
    console.log(`${chalk.bgWhite.black("[Log]")} Creating folder: ${chalk.yellow("./assembly/__tests__/")}`);
  ...
  if (!existsSync(typesFileLocation)) {
    console.log(`${chalk.bgWhite.black("[Log]")} Creating file: ${chalk.yellow("./assembly/__tests__/as-pect.d.ts")}`);
  ...
  if (!existsSync(configFileLocation)) {
    console.log(`${chalk.bgWhite.black("[Log]")} Creating file: ${chalk.yellow("./as-pect.config.js")}`);
  ...
  if (!existsSync(asconfigFileLocation)) {
    console.log(`${chalk.bgWhite.black("[Log]")} Creating file: ${chalk.yellow("./as-pect.asconfig.json")}`);
