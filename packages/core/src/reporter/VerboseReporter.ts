...
export class VerboseReporter implements IReporter {
  ...
  private writeTestFinish(test: SuiteTestReport): void {
    if (test.pass) {
      ...
      this.stdout!.write(
        test.negated
          ? ` ${ansi.green(` [Throws]: ✔`)} ${test.name}${rtraceDelta}\n`
          : ` ${ansi.green(`[Success]: ✔`)} ${test.name}${rtraceDelta}\n`,
      );
    } else {
      this.stdout!.write(`    ${ansi.red(`[Fail]: ✖`)} ${test.name}\n`);
      ...
      /* istanbul ignore next */
      if (test.message) {
        this.stdout!.write(` [Message]: ${ansi.yellow(`${test.message}`)}\n`);
      }
  ...
  private writeReport(report: SuiteReport): void {
    ...
    const result = report.pass ? ansi.green("✔ PASS") : ansi.red(`✖ FAIL`);

    const count = report.testCount;
    const successCount = report.testPassCount;

    const failText = count === successCount ? `0 fail` : ansi.red(`${(count - successCount).toString()} fail`);

    // There are currently no warnings provided by the as-pect testing suite
    /* istanbul ignore next */
    for (const warning of report.warnings) {
      /* istanbul ignore next */
      this.stdout!.write(`\n${ansi.yellow(` [Warning]`)}: ${warning.type} -> ${warning.message}\n`);
      /* istanbul ignore next */
      const stack = warning.stackTrace.trim();
      /* istanbul ignore next */
      if (stack) {
        /* istanbul ignore next */
        this.stdout!.write(`${ansi.yellow(`   [Stack]`)}: ${ansi.yellow(stack.split("\n").join("\n      "))}}\n`);
      }
      /* istanbul ignore next */
      this.stdout!.write("\n");
    }
    ...
    for (const error of report.errors) {
      this.stdout!.write(`\n${ansi.red(`   [Error]`)}: ${error.type} ${error.message}`);
      this.stdout!.write(
  ...
  [Groups]: ${ansi.green(`${report.groupPassCount} pass`)}, ${report.groupCount} total
  [Result]: ${result}
[Snapshot]: ${snapshotStats.total} total, ${snapshotStats.added} added, ${snapshotStats.removed} removed, ${snapshotStats.different} different
 [Summary]: ${ansi.green(`${report.testPassCount} pass`)},  ${failText}, ${report.testCount} total
...
${"~".repeat(80)}\n\n`);
  ...
  public onTodo(groupOrTodo: TestNode | string, maybeTodo?: string): void {
    ...
    /* istanbul ignore next */
    this.stdout!.write(`    ${ansi.yellow(`[Todo]:`)} ${todo}\n`);
  }
  ...
  public onLog(logValue: ReflectedValue): void {
    ...
    /* istanbul ignore next */
    if (stack) {
      this.stdout!.write(`   ${ansi.yellow(`[Stack]:`)} ${stack.trimStart().split("\n").join("\n        ")}\n`);
    }
  }
}
