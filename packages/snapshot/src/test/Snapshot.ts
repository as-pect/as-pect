...
export class Snapshot {
  ...
  public static fromString(data: string): Snapshot {
    if (typeof data !== "string") throw new Error("Cannot create snapshot from string when data is not a string.");
    const result = new Snapshot();
  ...
  public compareTo(other: Snapshot): SnapshotComparison {
    if (!this.stringified || !other.stringified) throw new Error("Cannot compare snapshots when Snapshot was not initialized.");
    const results = diff.diffLines(this.stringified, other.stringified, {
      ignoreWhitespace: true,
    });
    return new SnapshotComparison(results);
  }
}
